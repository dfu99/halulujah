...
| Cluster permutation tests (audit §6t, §6u) | Strict-shuffle (n=5000): combined p = 0.001 (rejects null of "30 cells are iid across the grid"). Within-row shuffle (preserves SS_rows + Q-clustering, tests helper effect): **p = 0.65** — confirms §6r helper insignificance non-parametrically. Within-column shuffle (preserves SS_cols, tests primary effect): **p < 0.0001** — confirms primary effect non-parametrically. The §6t strict-shuffle p decomposes 100% into the primary effect. |
| Per-question helper analysis (audit §6s, §6v) | At the per-question level, 49.2% of questions (123/250) show helpers disagree on correctness. However, the disagreement is *unstructured*: pooled Spearman ρ between helper correctness and helper own-domain self-solo accuracy across 123 disagree-questions is **ρ = +0.064** (P(ρ > 0) = 49.6%, indistinguishable from chance under H0 of no effect). Helper identity contributes per-question noise but no per-question signal that tracks helper competence. Reconciles §6h's between-helper aggregation Pearson r = +0.51 (n=5) with §6r's helper main effect insignificance (F = 0.96). |
| Triangulation summary (locked 2026-05-05) | **Primary effect: rejected H0 in 4/4 tests** — within-cell bootstrap CI [1.89, 8.39] excludes 1; question-clustered bootstrap CI [2.20, 7.45] excludes 1; ANOVA F(4, 1470) = 26.55 p < 1e-10; within-column cluster permutation p < 0.0001. **Helper effect: not rejected in 4/4 tests with helper resolution** — ANOVA helper main F = 0.96 p = 0.44; within-row cluster permutation p = 0.65; col-mean Pearson r = +0.51 (aggregation level only); per-q Spearman ρ = +0.064 P(ρ > 0) = 49.6%. |
| Figure          | `figures/audit-2026-05-05.png` (Panel B heatmap, Panel D sensitivity, Panel R bootstrap CIs, Panels S/T/U for §6n/§6o/§6p, Panel Q variance pie, Panels V/W/X for §6s/§6t/§6r ANOVA F-stats). |
| Source          | `tasks/audit-2026-05-05.md` §6b, §6c, §6f, §6m, §6n, §6o, §6p, §6q, §6r, §6s, §6t, §6u, §6v |
| Defensible headline (post-§6v lock) | *"On the verified Qwen3-1.7B LoRA pair-grid (5 primary × 6 helper × 50 question = 1500 observations), two-way ANOVA with replicates attributes 6.6% of variance to primary identity (F(4, 1470) = 26.55, p < 1e-10) and 0.3% to helper identity (F = 0.96, p = 0.44); cluster-permutation tests, question-clustered bootstrap CIs (4.47× cell-mean spread ratio, 95% CI [2.20, 7.45]; 22.1× cell-mean variance ratio), and a per-question helper-correctness analysis (pooled Spearman ρ = +0.064 between helper correctness and helper self-solo on 123 disagree-questions) all converge: in this setting, conversation outcome is determined by which specialist holds the question, not by which helper is in the room."* |
| Effect-size standardization (audit §6w) | Cohen's f = 0.269 (medium per Cohen 1988) for primary; f = 0.057 (trivial, below 0.10 small threshold) for helper; f = 0.106 (small) for interaction. Bias-corrected ω² = 0.064 primary, 0.000 helper, 0.000 interaction. |
| Oracle ceiling + orchestration (audit §6x, §6aa) | Best-of-6-helpers oracle = 72.4% pooled vs actual 51.0% = +21.4 pp gap. Best-by-col-mean per-primary rule closes 33% of the gap (+7.0 pp lift to 58.0%); majority-vote-of-6 closes 20% (+4.2 pp); always-base, always-self-match, best-self-solo, subject-aware all UNDERPERFORM random. **Best-by-col-mean reveals 4 of 5 primaries' best helper is CROSS-DOMAIN** (medicine→law, biology→law, law→medicine, physics→biology); only math→base. Self-match is never best in any primary's row. |
| Specialist-jackknife (audit §6y) | LOO range 10.37–31.33× (mean 17.67×, std 8.38) on the cell-mean variance ratio. Law specialist has Δ = −11.74 max-leverage; dropping it collapses ratio by more than half. Three converging analyses (§6c, §6o, §6y) confirm law is the single largest source of magnitude. |
| Per-cell additive-model residual (audit §6z) | 0/30 cells deviate from additive-model prediction at uncorrected p < 0.05, Bonferroni p < 0.05/30, or BH-FDR q = 0.05. The §6e flagged "biology helper at −6 pp on math primary" outlier is z = −1.51, p = 0.13 (within sampling noise). The additive model `(primary + helper)` fits perfectly; no primary × helper interaction structure exists. |
| Defensible headline (post-§6aa lock, FIFTH REVISION) | *"On the verified Qwen3-1.7B LoRA pair-grid (5 primary × 6 helper × 50 question = 1500 paired observations), replicate-aware two-way ANOVA attributes 6.6% of total variance to primary identity (F(4, 1470) = 26.55, p < 1e-10; Cohen's f = 0.269, 'medium') and 0.3% to helper identity (F = 0.96, p = 0.44; Cohen's f = 0.057, 'trivial'); cluster-respecting permutation, question-clustered bootstrap, and per-cell additive-model residual testing all converge on the same reading. Conversation outcome is determined by which specialist holds the question, not by which helper is in the room — although a per-primary helper-selection rule ('best-by-col-mean') closes 33% of the +21.4 pp oracle ceiling, with cross-domain helpers preferred over domain-matched helpers in 4 of 5 primaries."* |
| Difficulty stratification (audit §6cc, §6dd, §6ee, §6ff, §6gg, §6hh) | Hard subset (n=176, primary solo wrong) primary effect: cell-mean WHO 67.69× vs full 22.11×; ANOVA F_primary = 31.22, p < 1e-23; Cohen's f = 0.349; specialist-jackknife range 15.58–177.14×; bootstrap P(hard > easy) = 99.9%. Easy subset (n=74) primary effect: cell-mean WHO 1.50×, CI [0.23, 8.36] (includes 1.0). The WHO-asymmetry is concentrated on hard questions. |
| Conditional rates (audit §6hh) | Per-primary mean W2C rate (recovery from solo wrong) ranges from 20.8% (math) to 64.0% (biology); per-helper W2C is flat at 35-41%. Net corrector score (W2C − C2W): biology +51.7 pp, physics +17.9, medicine +16.7, math +3.2, law −8.1. Primary spread / helper spread on W2C = **7.07×**. Law primary is the only net-distractor row. |
| Cell-level helper roles (audit §6bb, §6oo) | 24/30 cells (80%) are corrector cells (w2c > c2w). All 6 non-corrector cells are in the law primary's row. Per-cell bootstrap CI: 8/30 cells robustly positive net corrector (CI excludes 0), 0/30 robustly negative. Biology row uniquely robust at every level (6/6 cells, full row, full bootstrap). |
| Mutual-unrecoverability mechanism (audit §6pp, §6qq) | 47.7% of hard questions are nearly unrecoverable (≤1 of 6 helpers help); 38.6% mutually unrecoverable (0/6 help). Math primary 53% mutually unrec, biology 19%. Within math primary: high_school_mathematics 75% unrec, college_mathematics 75%, abstract_algebra 50%, elementary_mathematics 20% — the §6cc primary effect on hard is heavily concentrated in math primary's HS/college math subjects. |
| Subject-stratified WHO ratio family (audit §6rr, §6ss, §6uu) | **Subject (17 levels) replaces primary (5 levels) as row factor in ANOVA. Full grid: 21.7× ≈ 22.1× primary. Hard subset: 56.5× ≥ 50.3× primary (subject STRENGTHENS). Easy subset: 1.64× ≈ 1.31× primary (both small).** SS_helper: 3.3% full / 1.4% hard / 24.3% easy. Cohen's f subject: 1.61 full, 2.01 hard, 0.82 easy. The "row factor dominates helper" finding is robust to (a) primary↔subject row factor choice (<14% relative ratio shift), (b) restriction to hard subset (amplifies 30-40×). Helper variance only matters on easy questions. |
| Per-subject hard W2C bootstrap CI (audit §6tt) | 17 subjects (n_hard ≥ 4) × 136 pairwise tests. Per-subject 95% CI: hs_biology W2C [50.8, 81.8] (n=22), college_math [0, 12.5] (n=4) — 39 pp non-overlap is the cleanest robust subject-level contrast. Pairwise: 32 uncorrected, 20 BH-FDR, 0 Bonferroni-136 (precision floor). Cross-primary biology-vs-math/law: 5/10 BH-FDR (3 hs_biology pairs all pass). |
| Per-subject easy C2W bootstrap CI (audit §6vv) | 5 subjects (n_easy ≥ 4) × 10 pairwise tests. Per-subject 95% CI: hs_biology C2W [3.3, 21.1], elementary_math [5.6, 22.2], college_biology [4.2, 29.2], prof_medicine [2.8, 44.4], prof_law [16.7, 52.8]. Pairwise: 2 uncorrected (hs_bio vs prof_law p=0.022; elementary_math vs prof_law p=0.029), **0 BH-FDR**, 0 Bonferroni-10. The §6uu C2W hierarchy is direction-correct but NOT statistically robust at subject granularity — confirms easy-regime is correlative while hard-regime is the audit's statistical backbone. |
| Defensible headline (post-§6tt lock, EIGHTH REVISION) | *"On the verified Qwen3-1.7B LoRA pair-grid (5 primary × 6 helper × 50 question = 1500 paired observations), the variance-decomposition ratio (row factor / helper) is **22.1× under primary-stratification (5 levels) and 21.7× under subject-stratification (17 MMLU subjects, n≥5)** — essentially identical. On the n=176 hard-question subset (primary solo wrong), the ratios rise to **50.3× primary / 56.5× subject** with helper variance shrinking to 1.4-1.7%. ANOVA: F_primary = 26.55 (p < 1e-21, f = 0.27 medium) on full grid; 31.22 (p < 1e-23, f = 0.35) on hard. Per-primary mean recovery from wrong (W2C) ranges 21% (math) to 64% (biology) while per-helper W2C is flat at 35-41% (primary spread / helper spread = 7.07×). Per-subject hard W2C bootstrap CI: hs_biology [50.8%, 81.8%] does not overlap college_math [0%, 12.5%] — a 39 pp robust gap that survives BH-FDR among 136 subject-pair tests. Conversation outcome is determined by the primary's willingness to update from wrong, not by helper identity — robust to whether the row factor is primary identity (5 levels) or subject identity (17 levels), to restriction to hard questions, and to formal CI placement at the most-granular stratification level the data support."* |
| High-n bootstrap on subject-pair Bonferroni-136 (audit §6ww) | At n_iter=50000 (vs §6tt n=2000 precision floor 0.001), 11 of 136 subject-pair contrasts survive Bonferroni-136 at α=0.05. The §6tt 0/Bonferroni was a precision-floor artifact. Headline: hs_biology > college_math by 62.5 pp; hs_biology > prof_law by 47.9 pp; **WITHIN math primary** college_math < elementary_math by 34.2 pp (Bonferroni-136-significant). |
| Easy-only ANOVA (audit §6xx) | F_primary collapses 8.5× from full to easy (26.55 → 3.13, p=0.0149); F_helper rises 2× (0.96 → 1.91, p=0.092 — strongest helper signal in audit, still non-rejecting). Row/helper F-ratio: full 27.6× → hard 62× → **easy 1.64×**. The WHO-asymmetry is strictly a hard-question phenomenon at the ANOVA level too. |
| High-n bootstrap on primary-pair Bonferroni-25 (audit §6yy) | At n=50000 same 2/25 Bonferroni survivors as §6kk n=2000 (biology > law +59.8 pp; biology > math -48.5 pp). The §6kk 2/Bonferroni was NOT a precision-floor artifact. Helper-pair: 0/15 Bonferroni at any n. **Formal-statistical hierarchy: subject 11/136, primary 2/10, helper 0/15** — strongest "no helper effect" finding. |
| 4-axis jackknife family (audit §6y, §6gg, §6zz, §6aaa) | All single-element drops keep row/helper ratio above 10×. Full primary 10–31× (3.0× factor); hard primary 16–177× (11.4×); hard subject (n=17) 26–86× (3.3×); hard helper (n=6) 46–364× (7.9× total / 1.24× excluding base — audit's tightest jackknife). Base helper drop produces 364× ratio explosion (vs 56.5× baseline) — the §6jj/§6yy "base lone outlier" finding has a variance-decomposition substrate. |
| Wilson closed-form CI corroboration (audit §6bbb) | Wilson 95% score interval is tighter than bootstrap for 16 of 17 subjects (mean Δ width = -20.3 pp). The §6tt bootstrap is uniformly conservative; underlying signal is at least as strong as bootstrap CIs suggest. The §6tt 20 BH-FDR survivors and §6ww 11 Bonferroni-136 survivors are LOWER BOUNDS on what closed-form parametric tests would find. |
| Closed-form pairwise z-test (audit §6ccc, §6ddd) | Two-proportion z-test (pooled variance) on 136 subject pairs: **28 of 136 Bonferroni-136 survivors** (vs bootstrap 11). 8-pair INTERSECTION = audit's IRONCLAD set. Helper-pair version (§6ddd, 15 tests): **0 of 15 even uncorrected α=0.05** (most-significant pair: base vs medicine, p=0.150). Same closed-form test, same data, same correction — row 28/136 Bonferroni; helper 0/15 EVEN UNCORRECTED. The audit's cleanest within-experiment row-vs-helper disambiguation. |
| Defensible headline (post-§6ddd lock, ELEVENTH REVISION — CURRENT) | *"On the verified Qwen3-1.7B LoRA pair-grid (5 primary × 6 helper × 50 question = 1500 paired observations), pairwise tests at three aggregation levels under both cluster-respecting bootstrap and closed-form two-proportion z-test give a clean formal hierarchy: **subject-pair (136 tests): 11 bootstrap / 28 z-test Bonferroni survivors; primary-pair (10 tests): 2 / 2; helper-pair (15 tests): 0 / 0 even uncorrected**. Variance-decomposition row/helper ratio is 22.1× / 21.7× on full grid; 50.3× / 56.5× on hard; 1.31× / 1.64× on easy. Replicate-aware ANOVA F_primary = 26.55 / 31.22 / 3.13 (p < 1e-21 / 1e-23 / 0.015) across the difficulty triple. F_helper non-rejecting at every stratification (max 1.91, p = 0.092 on easy). Per-primary mean W2C ranges 21% (math) to 64% (biology) while per-helper W2C is flat at 35-41% (primary spread / helper spread = 7.07×). hs_biology hard W2C CI [50.8%, 81.8%] does not overlap college_math [0%, 12.5%] under any of three CI methods (bootstrap, Wilson, z-test). **Conversation outcome is determined by the primary's willingness to update from wrong, not by helper identity** — robust to (a) primary↔subject row factor choice, (b) hard-subset restriction, (c) the most conservative family-wise multiple-comparison correction, (d) the bootstrap precision floor, AND (e) the choice of CI / pairwise test method (bootstrap vs Wilson vs closed-form z-test)."* |
| Triangulation table (post-§6ddd, FINAL) | **35 row-effect tests; 8 helper-effect lenses (all non-rejecting).** All 35 row-side tests reject H0 of no row effect at every stratification level. Helper-side: F_helper at most 1.91 (easy ANOVA, p=0.092 — never rejects); 0/15 Bonferroni-15 at any iteration count under bootstrap; 0/15 even uncorrected under closed-form z-test. See `tasks/audit-2026-05-05.md` §10 11th revision for the canonical paragraph. |

## C11. Drift study — training time effect on collaboration is helper-side asymmetric  *(NEW 2026-05-09)*

| Item | Source / Number |
|------|-----------------|
| Headline | Across cp1500 → final SFT training (~5× scale difference in tokens), pair-grid accuracy improves by global mean **+6.5 pp** with no primary regressing. The drift WHO-asymmetry ratio (var-of-row-mean-deltas / var-of-col-mean-deltas on the delta matrix) is **0.23×** — the *opposite* asymmetry from the static grid's 52×. Training time effect on collaboration varies more by helper identity than by primary identity. |
| Per-primary Δ acc | medicine +9.3 pp, math +6.3 pp, biology +5.7 pp, law +4.7 pp, physics +6.3 pp. Range 4.7-9.3 pp. |
| Per-helper Δ acc | medicine +7.6, math +6.8, biology +7.6, law +10.8, physics +6.0, **base +0.0**. The base-helper-zero finding: training time adds 0 pp to pair-grid accuracy when the helper is the un-trained base model; only FT-specialist-helper cells gain. |
| Reading | Static outcome (§14d, C10) is primary-side asymmetric (who holds the question dominates). Change in outcome across training time is helper-side asymmetric (which helper you chose dominates how much you benefit). The collaborative improvement is *jointly produced* by FT-FT pairs — training improves how specialists *talk to each other*, not how they speak alone. |
| Switch deltas | Δ W2C = +2.73/cell (training adds recovery); Δ C2W = +0.20/cell (essentially unchanged). Training-time corruption rate is flat; recovery rate increases. |
| Datafile | `results/ft_pair_grid_step1500_2026-05-08/matrix_results_v2.json` (cp1500 grid, 35 cells) + `results/ft_pair_grid_2026-05-08/matrix_results_v2.json` (final grid). Drift summary at `results/ft_pair_grid_2026-05-08/drift_summary.json`. |
| Figure | `figures/drift_step1500_vs_final_2026-05-08.png` (3-panel heatmap: Δ accuracy, Δ W2C, Δ C2W per-cell). |
| Audit trail | `tasks/audit-2026-05-05.md` §15 (motivation, headline numbers, base-helper-zero finding, asymmetry-inversion reading, datafile pointers, caveats). |
| Caveats | (1) Two step points only (cp1500, final); a real drift trajectory would need ≥3 step points. (2) CI on the 0.0pp base-helper delta needs computing. (3) Math primary remains the worst at both points; training does not rescue math from its SFT-format overfit. |

## C10. WHO-asymmetry replicates and amplifies under Full-FT  *(NEW 2026-05-08)*

| Item | Source / Number |
|------|-----------------|
| Headline | On the **Full-FT** Qwen3-1.7B 5×6 pair-grid (5 specialists trained with no rank constraint, same per-domain MMLU splits as the LoRA roster), the WHO-asymmetry ratio is **52.37×** (var-of-row-means / var-of-col-means estimator). Same estimator on the LoRA grid: 16.93×. Helper-side flatness is preserved: max-min spread across 6 helpers is 5.6 pp under Full FT vs 6.8 pp under LoRA. The asymmetry is therefore *not* a low-rank/intruder-dimension artifact. |
| Per-primary mean (Full FT vs LoRA) | medicine 0.667 vs 0.510 (+15.7 pp); math 0.273 vs 0.430 (-15.7 pp); biology 0.723 vs 0.730 (≈0); law 0.437 vs 0.370 (+6.7 pp); physics 0.507 vs 0.510 (≈0). Spread = 45 pp Full FT, 36 pp LoRA. |
| Per-helper mean (Full FT vs LoRA) | medicine 0.528/0.536; math 0.544/0.544; biology 0.512/0.492; law 0.552/0.536; physics 0.504/0.484; base 0.488/0.468. Helpers lie within 5.6 pp of each other under Full FT (vs 6.8 under LoRA). Helper-side variance is *not* attenuated when rank constraint is removed. |
| Switch-type totals across 30 pair cells (1500 q) | LoRA c2w=82, w2c=625 (ratio 0.13); Full FT c2w=135, w2c=574 (ratio 0.24). Both training methods produce strongly net-corrective collaboration; Full FT specialists are slightly more flip-prone (24% c2w/w2c vs 13%) but still net-helpful. |
| Specialist verification | 5-shot MMLU per-checkpoint (audit §14b): mean acc base 0.564; medicine-final 0.592 (PASS gate); math-final 0.516 (FAIL); biology-final 0.556 (PASS); law-final 0.504 (FAIL); physics-final 0.580 (PASS). 3/5 pass strict +5pp OOD gate. Math+law fail on 5-shot raw format but operate in chat-template/CoT format — captured as solo cells in the pair-grid. |
| Datafile | `results/ft_pair_grid_2026-05-08/matrix_results_v2.json` (parser-repaired; original at `matrix_results_orig.json`). 35 cells; 50 q × 3 rounds full-CoT each. |
| Figures | `figures/ft_vs_lora_pair_grid_2026-05-08.png` (per-primary bar charts, LoRA top row vs Full FT bottom row). `figures/ft_vs_lora_who_asymmetry_2026-05-08.png` (asymmetry-ratio comparison bar chart). |
| Audit trail | `tasks/audit-2026-05-05.md` §14 (full ckpt-by-ckpt scan, gate results, full per-cell matrix, switch counts, caveats). |
| Caveats | (1) ckpts used are domain-final, not matched-solo (queued follow-up). (2) Bootstrap CI on the 52.37× ratio queued. (3) Math primary's collapse to 0.27 is an SFT-format-overfit artifact partially distinct from the asymmetry: the model emits long CoT that rarely lands on a parseable letter; the v2 parser (extended pattern set: `\boxed{X}`, "the answer is X", `\textbf{X}`, last-occurrence-wins) recovers ~25% of these but math row remains the worst. |
