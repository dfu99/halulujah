"""Verify a medicine LoRA adapter on top of a base model.

Compares base-alone vs base+adapter on:
  - 6 MMLU medicine subjects (5-shot, n=100 each by default)
  - MedQA-USMLE-4-options test split (5-shot, n=200 by default, held-out)

Usage:
  python verify_medicine_adapter.py \\
    --base Qwen/Qwen3-1.7B \\
    --adapter /workspace/adapters_1p7b_ood/r16/adapter_medicine_medqa \\
    --output results/specialist_verification/medicine_r16.json
...
def _format_medqa_q(ex):
    options = ex["options"] if isinstance(ex["options"], dict) else eval(ex["options"])
    parts = [ex["question"].strip()]
    for k in sorted(options.keys()):
        parts.append(f"{k}) {options[k]}")
    return "\n".join(parts)


def eval_medqa(model, tok, n_questions, n_shot, cache_dir):
    from datasets import load_dataset
    train = load_dataset(
        "GBaker/MedQA-USMLE-4-options", split="train", cache_dir=cache_dir)
    test = load_dataset(
        "GBaker/MedQA-USMLE-4-options", split="test", cache_dir=cache_dir)
    test_qs = list(test)[:n_questions]
    fewshots = list(train)[:n_shot]

    correct = 0
    for q in test_qs:
        question = _format_medqa_q(q)
        gold = q["answer_idx"]
        prompt_parts = [
            f"Question: {_format_medqa_q(fs)}\nAnswer: {fs['answer_idx']}"
            for fs in fewshots]
        prompt_parts.append(f"Question: {question}\nAnswer:")
        prompt = "\n\n".join(prompt_parts)
        inputs = tok(prompt, return_tensors="pt", truncation=True,
...
def evaluate(model, tok, args, label):
    out = {"label": label, "subjects": {}, "medqa": None}
    ...
    if args.medqa_n > 0:
        acc, n = eval_medqa(model, tok, args.medqa_n, args.n_shot,
                            args.cache_dir)
        out["medqa"] = {"accuracy": acc, "n": n}
        print(f"  {label} MedQA-test: {acc*100:.1f}% n={n}")
    return out
...
def main():
    ...
    p.add_argument("--mmlu-subjects", nargs="+", default=MEDICINE_SUBJECTS)
    p.add_argument("--output", required=True)
    p.add_argument("--skip-base", action="store_true",
                   help="Skip base eval (reuse cached base result)")
    p.add_argument("--base-results",
                   help="Optional path to JSON with cached base eval to merge")
    args = p.parse_args()
    ...
    if args.skip_base and args.base_results:
        base = json.load(open(args.base_results))["base"]
    else:
    ...
    if spec.get("medqa") is not None and base.get("medqa") is not None:
        b = base["medqa"]["accuracy"] * 100
        sp = spec["medqa"]["accuracy"] * 100
        d = sp - b
        if d >= 5: pass_count += 1
        print(f"{'MedQA-test':32s} {b:>7.1f}% {sp:>7.1f}% {d:>+7.1f}")
        n_bench += 1
