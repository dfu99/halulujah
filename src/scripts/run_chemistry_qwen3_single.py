"""Single-rank training + verification for chemistry specialist on Qwen3-1.7B.

Trains LoRA at rank=16 on SciQ (general science MCQ, 4-option), then
verifies on 2 MMLU chemistry subjects + optional MMLU-Pro/chemistry +
SciQ test held-out.

SciQ is general science (physics + biology + chemistry); the specialist
is technically a "science" specialist. We eval on chemistry subjects to
test the chemistry-side transfer. Mirrors run_physics_qwen3_single.py
exactly — only the verifier and output paths change.

Usage:
  python run_chemistry_qwen3_single.py --rank 16
"""
import argparse
import json
import os
...
def main():
    ...
    p.add_argument("--domain", default="chemistry")
    ...
    p.add_argument("--adapter-root",
                   default="/workspace/adapters_1p7b_ood/chemistry_qwen3")
    p.add_argument("--results-dir",
                   default="results/specialist_verification/chemistry_qwen3")
    p.add_argument("--log-dir", default="logs/chemistry_qwen3")
    ...
    output = Path(args.results_dir) / f"chemistry_r{args.rank}.json"
    ...
    verify_cmd = [
        sys.executable, str(ROOT / "src/scripts/verify_chemistry_adapter.py"),
        "--base", args.base,
        "--adapter", str(adapter_path),
        "--mmlu-n", str(args.mmlu_n),
        "--sciq-n", str(args.sciq_n),
        "--mmlu-pro-n", str(args.mmlu_pro_n),
        "--cache-dir", args.cache_dir,
        "--output", str(output),
    ]
