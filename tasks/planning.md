...
- 2026-05-13: **obj-parser-codification-2026-05-13** — PI flagged that strip-reasoning pollution had bit three grids in a row. Codified the letter parser in commit `f60d5e0`: (a) promoted permissive patterns from `recompute_pre_a_letters.py` into the canonical `extract_answer_letter`; (b) added `assert_letter_extraction_quality(per_q, max_x_rate=0.05)` hard-gate to `cell_pair_grid_eval.py`; (c) 21 golden tests; (d) lessons.md "Letter parser is load-bearing" + default-to-A sub-lesson. Audit findings: **5.21× published 4B Full FT WHO ratio is INVALID** — the prior v2 repair had 51.5% default-to-A bias (vs 26% ground-truth A); honest WHO = 3.11×. 1.7B Full FT *strengthens* after recovery from `final_raw`: 38.79× → 54.74×, 233 letters recovered, X-rate 36% → 20.5%. 1.7B LoRA and 4B Full FT both need GPU re-runs (no raw text preserved). Figures: `figures/parser_audit_who_ladder_2026-05-13.png`, `figures/parser_recovery_1p7b_ft_2026-05-13.png`. **Paper-claim impact**: C9 (WHO-asymmetry) survives at 1.7B FT (54.74×), invalid at 4B as published. Headline 52.37× FT vs 16.93× LoRA comparison at 1.7B needs 1.7B LoRA re-run before paper-ship.

- 2026-05-10: **4B bio Full FT corruption recovered (obj-4b-bio-corruption-recovery-2026-05-10)** — second partial-zero corruption (52/398 tensors at layers 31-35) diagnosed and recovered. cp-1500 (80% of planned 1875 steps) passes per-tensor scan (398/398) and 5-shot gate: med −2, math +6, bio −2, law +6, phys −2 (vs Qwen3-4B base). All 5 4B Full FT specialists now verified. **4B Full FT 5×6 pair-grid (35 cells, 50 q × 3 rounds, full-CoT, `--base-name Qwen/Qwen3-4B`) launched on A40 pod**, output dir `results/ft_pair_grid_4b_2026-05-10/`. Next in queue: 4B LoRA r=8 + r=64 training chains, then their pair-grids. Lessons added: moosefs per-pod quota ~21 GB even after 60 GB container upgrade; 5-shot wrapper has 1.7B-default --output-dir trap. Figure: `figures/4b_verification_gate_2026-05-10.png`.
...
- 2026-05-08: **obj-ft-pair-grid-2026-05-08** — Full FT 5×6 pair-grid landed (35/35 cells, 1750 q, 50 q × 3 rounds × full-CoT each). **WHO-asymmetry = 52.37× under Full FT vs 16.93× under LoRA (same estimator) — 3.1× amplification when rank constraint is removed.** Per-helper means within 5.6 pp of each other. Switch-type totals: LoRA c2w=82, w2c=625; Full FT c2w=135, w2c=574 — both strongly net-corrective. Closes audit's main reviewer concern about LoRA-vs-FT confound. Artifacts: `results/ft_pair_grid_2026-05-08/`, `figures/ft_vs_lora_*_2026-05-08.png`, `paper/{ft_extension,example_traces}_2026-05-08.md`, `tasks/audit-2026-05-05.md` §14, `paper/claim_evidence_map.md` C10.
- 2026-05-08: **obj-per-ckpt-mmlu-bench-2026-05-08** — 30/30 ckpt × 5-domain × 50-q 5-shot MMLU benchmark. Verification gate (≥+5pp on ≥1 OOD): 3/5 pass (medicine, biology, physics); math+law fail strict 5-shot but operate normally in CoT pair-grid. Bio-final corruption (per-tensor zero in layer 17+/norm) diagnosed via per-tensor scan and restored from cp7500. New lessons.md entry: per-tensor integrity scan after any rsync-streaming pipeline.
- 2026-05-07: **obj-full-ft-chain-2026-05-07** — All 5 Full FT specialists trained with full per-step ckpts (29 model.safetensors, ~108 GB on WD_BLACK). 7h 49m unattended. 5 dep drifts + 2 race/quota bugs caught + fixed.
